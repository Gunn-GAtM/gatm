...
pdflatex -shell-escape -interaction=nonstopmode -file-line-error --output-directory=build gatm_answers.tex | grep ".*:[0-9]*:.*"
...
rm *.pdf *.asy *.log *.aux gatm_answers-*.tex *.pre >/dev/null 2>&1
