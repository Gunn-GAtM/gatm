...
numbers = [
    ...
    ("\\frac{\pi}{4}", decimal.Decimal('0.7853981633974483096156608458198757210492923498')),
    ...
    ("\\frac{\\sqrt{2}}{10}", decimal.Decimal("0.1414213562373095048801688724209698078569671875"))
]
...
for i,pair in enumerate(numbers):
    table[i].extend(["$r_{%s}$" % (i + 1), '$%s$' % pair[0], "\\tcg{0.}" + ('' if i != 0 else "\\Aboxed{")])
    expansion = str(pair[1])[2:digits+2].ljust(digits, '0')
    table[i].extend(('%s}' if i == j else ("\\tcg{%s}" if i != j + 1 else "\\tcg{%s} \\Aboxed{")) % char for j,char in enumerate(expansion))
    table[i].append("$\\cdots{}$")

for i in range(digits+3):
    table[-2].append("$\\vdots{}$" if (i < 2 or i % 2 == 0) else '') # so it doesn't get too cramped
...
for i in range(digits):
    ...
    table[-1].append("$%s$" % ((int(str(numbers[i][1]).ljust(digits, '0')[2+i]) + 1) % 10))
