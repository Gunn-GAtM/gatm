...
% Brandon to Timothy: stop using dashes wrong. NEVER PUT spaces around an em dash. this hurts my eyes. LOVE U BULBY
...
These six positions, along with the operation of multiplication, form another group: the \textbf{dihedral group} of the equilateral triangle, or $D_3$. Again, like a snap group, it is denoted $D_3$ because it is a dihedral group of a ($3$-sided) triangle, not the group size. If we split ``dihedral'' into ``di-'' and ``-hedral,'' we see it means ``two faces''; these are the two faces of our paper triangle. Dihedral groups exist for any plane figure; mathematically speaking, the dihedral group $D_n$ is the group of symmetries, or \textbf{symmetry group}, of a $n$-sided regular polygon. A symmetry group is the set of all isometries of an object, including the identity, which map an object to itself. Note that the symmetry groups of plane figures are specifically called dihedral groups, while shapes in any number of dimensions have symmetry groups. Let's study the properties of the dihedral group of the equilateral triangle.
