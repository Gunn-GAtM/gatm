...
Notice that each position corresponds to a different operation which preserves the triangle's location. For example, $I$ means ``leave the triangle alone,'' $A$ means ``flip the triangle about the $A$ axis,'' and $D$ means ``rotate the triangle $120\degree{}$ counterclockwise.'' We can combine these operations to form other operations by writing them in sequence. Unlike most cases, however, we evaluate them right-to-left (RTL) rather than left-to-right (LTR). For example, $AD=B$, as shown in Figure~\ref{fig:aid_is_b}.
