...
\begin{inner_problem}
\item Provided that $q^2\leq p^3$, show that $y=2\sqrt{p}\cos\left(\theta + \frac{2\pi n}{3})\right)$, where $n$ is an integer. Why does this yield all three solutions?
\end{inner_problem}

This isn't actually hard. We know that $\cos 3\theta = 4c^3-3c = \frac{q}{p^{3/2}}$, so there are three possible values for $\cos\theta = c$. Namely, if $\theta_0 = \cos^{-1} \frac{q}{p^{3/2}}$ is the principal value, then we also have unique solutions

$$\theta_1 = \frac{2\pi}{3} + \theta_0, \, \theta_2 = \frac{4\pi}{3} + \theta_0,$$

because multiplying these by $3$ to get $\theta$ just adds a multiple of $2\pi$. Indeed, we have $$c = \cos\left(\theta + \frac{2\pi n}{3}\right)$$ as a solution for any integer $n$. Substituting into the expression for $y$, we get

$$y=2c\sqrt{p}=2\cos\left(\theta + \frac{2\pi n}{3}\right)\sqrt{p},$$
