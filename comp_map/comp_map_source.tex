...
\item What does the size its determinant indicate?
\end{enumerate}
\end{enumerate}
...
\item What if we $\left[\begin{smallmatrix}b & d \end{smallmatrix}\right]=r\left[\begin{smallmatrix}a & c \end{smallmatrix}\right]$? That is, what if the second row of the matrix was a linear multiple of the first row?
\end{enumerate}
\end{enumerate}
\setcounter{problem_i}{\value{enumi}}
\end{enumerate}
...
\item Start by checking the determinant. If it is nonzero, continue to step b. Otherwise, you are done, because the inverse does not exist.\footnote{Why doesn't the inverse exist? Describe the mapping.}
