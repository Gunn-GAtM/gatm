...
\end{enumerate}
\end{multicols}
\item A \textbf{linear mapping} $f$ is one in which all lines are mapped to lines and the origin remains a fixed point. Algebraically, $f\left(\left[\begin{array}x \\ y \end{array}\right]\right)=xf\left(\left[\begin{array}1 \\ 0 \end{array}\right]\right)+yf\left(\left[\begin{array}0 \\ 1 \end{array}\right]\right).$ I claim that we can build any linear mapping of the plane by multiplying together some combination of the matrices from Problem~\ref{prob:list_transformations}. Only two classes of matrix, however, are necessary; all other matrices are products or examples of these. Which two classes of matrix do you think comprise the minimum set from which the others can be composed? Be able to justify your choice.
...
\item The difference of the products of the two diagonals of a $2\times 2$ matrix is called the \textbf{determinant} of the matrix, written $\det \left[\begin{array}a & c \\ b & d \end{array}\right]=ad-bc$. What does it measure?
...
\end{enumerate}
\item \begin{enumerate}
\item What would have happened to the parallelogram if $\left[\begin{array}c \\ d \end{array}\right]=k\left[\begin{array}c \\ d \end{array}\right]$?
\item What would its area be?
\item What would the determinant of the matrix be?
\item What if $\left[\begin{array}b & d \end{array}\right]=r\left[\begin{array}a & c \end{array}\right]$?
\end{enumerate}
\end{enumerate}
\setcounter{problem_i}{\value{enumi}}
\end{enumerate}
...
\item Start by checking the determinant. If it is nonzero, continue to step (ii). Otherwise, you are done.\footnote{Why?}
...
\noindent This process applied to the matrix $\left[\begin{array}a & c \\ b & d \end{array}\right]=\left[\begin{array}2 & 3 \\ 4 & 1 \end{array}\right]$, is shown in Figure~\ref{fig:undo_matrix} (some not to scale).
...
\item Now let's put this all together. Undo each of the operations in turn, until only matrix $\left[\begin{array}a & c \\ b & d \end{array}\right]$ remains on the left side. Remember that what you do on the left side of the expression must also be done to the right side, so on the right side you will see the basic operations from which $\left[\begin{array}a & c \\ b & d \end{array}\right]$ is composed. Order is important!
...
\end{enumerate}
\end{multicols}
\item Onto what line does $\left[\begin{array} a & b \\ 2a & 2b\end{array}\right]$ project the plane? Solve for $a$ and $b$ such that the matrix projects perpendicular onto the line. You can do this because you know that a point on the line should not move under the projection and a point on a line perpendicular to the line has its image on the origin. Using this information you can set up two equations with two unknowns. \label{prob:project_the_plane}
\item Use Problem~\ref{prob:project_the_plane} to decompose $\left[\begin{array} 2 & 3 \\ 4 & 6 \end{array}\right]$ into a projection to a line followed by a size change.
\item Decompose $\left[\begin{array} 1 & 2 \\ 3 & 6\end{array}\right]$ into a projection perpendicular to a line followed by a size change.
