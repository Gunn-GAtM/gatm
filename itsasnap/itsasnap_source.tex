...
As an example, $A\snap B = E$, as shown in Figure~\ref{fig:snap_ex}. Simply, think of it as a function composition $f \circ g = f(g(x))$, where $g(x)$ is applied first and its output acting as an input to $f(x)$.
