...
Figure~\ref{fig:all_3_cols} shows the $6$ ways the posts in $2$ rows, with each row containing $3$ posts, can be paired up. Convince yourself that these are the only $6$; they are labeled as $I$, $A$, $B$, $C$, $D$, and $E$.
...
More specifically, we will call this group the \textbf{snap group} of size $6$, or $S_3$. There are countless other mathematical groups, so we must be precise about which group we are talking about. Note that the snap group is denoted $S_3$, not $S_6$, because $3$ is the number of posts in each row, \textbf{not} the size of the group.
...
\begin{enumerate}
	\item Fill out a $6\times 6$ table like the one in Figure~\ref{fig:sbstable_CHANG}, showing the results of each of the $36$ possible snaps, where $X\snap Y$ is in $X$'s row and $Y$'s column. % replaced ~\ref{fig:sbstable} temporarily with 4 because it points to Figure 5 not 4
