...
You have studied geometry and complex numbers before so you should have a basis to start this investigation, but the other three topics may be a bit unfamiliar at this point.

Figure~\ref{fig:all_3_cols} shows the six ways the points in two rows, each with three points (later referred to as ``posts"), can be paired up. Convince yourself that these are the only six; they are labeled as $I$, $A$, $B$, $C$, $D$, and $E$.
Then, consider a grid of posts with $3$ rows and $3$ columns.
An elastic string is anchored to one post in the top row, looping around a post in the middle row, finally descending to a post in the bottom row.
...
As an example, $A\snap B = E$, as shown in Figure~\ref{fig:snap_ex}. Simply, think of it as a function composition $f \circ g = f(g(x))$ with $f=A$ and $g=B$, where $g(x)$ is applied first and its output acting as an input to $f(x)$.
...
More specifically, we will call this group under the $\snap$ operation the \textbf{snap group} of size $6$, or $S_3$; there are countless other mathematical groups, so we must be precise about which we are talking about. Note that the snap group is denoted because $S_3$, not $S_6$, because 3 denotes the number of posts in each row, not size of the group.
...
\begin{enumerate}
	\item Fill out a $6\times 6$ table like the one in Figure 4, showing the results of each of the $36$ possible snaps, where $X\snap Y$ is in $X$'s row and $Y$'s column. % replaced ~\ref{fig:sbstable} temporarily with 4 because it points to Figure 5 not 4
	...
	\item If you constructed a $5\times 5$ table using only $5$ of the snap elements, the table would not describe a group, because there would be entries in the table not in those $5$.
	      Therefore, a group must be \textbf{closed} under its operation; if $X,Y\in G$ ($\in$ means ``is in / elements of''), then $X\snap Y\in G$ for all $X,Y$.
