...
Yes! Let the group be called $\mathcal{2}=\{2^x\, :\, x\in \mathbb{Z}\}$ for fun. The identity element is $2^0=1$. The group properties are satisfied:

Identity: $2^x\cdot 1=1\cdot 2^x=2^x$.

Closure: $2^x\cdot 2^y = 2^{x+y} \in \mathcal{2}$.
...
It is isomorphic to integers under addition. $2^n\in \mathcal{2}$ corresponds with $n\in \mathbb{Z}$, since we have $$2^m\cdot 2^n=2^{m+n}\leftrightarrow m+n.$$
