...
Consider how we would choose a random point in $\mathcal{C}$. Starting at $C_1$, we can either go to the left segment or the right segment (marked as $L$ and $R$ in Figure~\ref{fig:construct_cantor}). If we choose $L$, then we once again have $2$ choices: to go to $L'$ or $R'$. This continues ad infinitum. Thus, we can correspond each element $x\in \mathcal{C}$ with a binary number in the interval $[0,1]$.
