...
\begin{figure}[h]
	...
	\begin{center}
		...
		\end{minipage}
        \begin{minipage}[t]{.45\textwidth}
            \caption{Failed attempt at a group table with $5$ elements.}
            \label{fig:incomplete_group}
        \end{minipage}
	\end{center}
...
\item If you constructed a group table using only $5$ of the snap elements, the table would not describe a group, because there would be entries in the table outside of those $5$. (See Figure~\ref{fig:incomplete_group} for a failed attempt at a group table, in which the element $A$ was omitted.) Indeed, a group must be \textbf{closed} under its operation: If we compose any two elements $X\snap Y = Z$, $Z$ must also be an element of the group. \textit{Some} subsets of our six elements, however, do happen to be closed among themselves. Write valid group tables using exactly one, two, and three elements from the snap group. These are known as \textbf{subgroups}.\label{prob:group_definition_end}
